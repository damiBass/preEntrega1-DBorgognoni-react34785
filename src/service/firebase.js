...
const firebaseConfig = {
  apiKey: "AIzaSyANSVjXrTlrdGMgazBUJrF-5b7e8Eu9IZ8",
  authDomain: "project-store-react34785.firebaseapp.com",
  projectId: "project-store-react34785",
  storageBucket: "project-store-react34785.appspot.com",
  messagingSenderId: "313096254966",
  appId: "1:313096254966:web:dd8f696fda6f3b7e0103d7",
};

const firebaseApp = initializeApp(firebaseConfig);
const DB = getFirestore(firebaseApp);

export async function getDetailItemProd(id) {
  try {
...
export async function getItemProd() {
  try {
    const collectionProducts = collection(DB, "products");
    let res = await getDocs(collectionProducts);

    const products = res.docs.map((docs) => {
      return {
        ...docs.data(),
        id: docs.id,
      };
...
export async function getItemProdCategory(category) {
  const productsRef = collection(DB, "products");
  const myQuery = query(productsRef, where("category", "==", category));

  const queryRes = await getDocs(myQuery);

  const emptyCat = queryRes.docs.length < 1;

  if (emptyCat) throw new Error("Categoria no encontrada");
...
// export async function exportItemsDB() {
//   const dataProducts = [
//     {
//       id: 1,
//       title: "iPhone 9",
...
//   for (let itemProd of dataProducts) {
//     delete itemProd.id;
//     const docRef = await addDoc(collectionRef, itemProd);
//     console.log(docRef);
//   }
// }

export async function getUpdateOrderStock(data) {
  const collectionProdRef = collection(DB, "products");
  const collectionOrderRef = collection(DB, "buyOrders");
  ...
  products.docs.forEach((itemInDB) => {
    let stock = itemInDB.data().stock;
    let itemInCart = data.items.find((item) => item.id === itemInDB.id);
    let stockUpdate = stock - itemInCart.quantity;
    batch.update(itemInDB.ref, { stock: stockUpdate });
  });

  const docOrderRef = doc(collectionOrderRef);

  batch.set(docOrderRef, data);
